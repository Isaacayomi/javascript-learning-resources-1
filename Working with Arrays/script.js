...
console.log('------TO ACESS THE CURRENT INDEX (POSITION) OF THE ARRAY USING THE FOR OF LOOP -----------')
...
console.log('------TO ACESS THE CURRENT INDEX (POSITION) OF THE ARRAY USING THE FOREACH METHOD -----------')
movements.forEach(function(currentMovement, index, array) {
    if (currentMovement > 0) {
        console.log(`Position ${index + 1}: You deposited ${currentMovement}`)
    } else {
        console.log(`Position ${index + 1}: You withdrew ${Math.abs(currentMovement)}`) //the Math.abs method is used to remove the negative sign in front of the value
    }
})
...
const uniqueValues = new Set(
    ['United States', 'United Kingdom', 'London', 'United States', 'London', 'India']
);
