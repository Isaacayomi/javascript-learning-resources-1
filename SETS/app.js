...
rest.set('name', 'Prime').set('age', 20).set('Stack', 'Frontend').set('Height', 5.8).set(true, 'We are open :D').set(false, 'We are closed :(').set('open', 11).set('close', 23)
console.log(rest)
...
//3.
console.log(`An event happend, an average, every ${90 / gameEvents.size} minutes`)

//4.
for (const [key, event] of gameEvents) {
    const decide = key <= 45 ? `[FIRST HALF] ${key}: ${event}` : `[SECOND HALF] ${key}: ${event}`
    console.log(decide)
}
